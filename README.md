...
- ⚙️ 配置管理优化：新增的autoLoadDefaultData属性，提高了默认数据加载的效率
  - 关闭可直接读取已加载过的缓存，但可能导致数据更新不及时
...
- 📝 优化文本系统：显示更加简洁，友好
- 📋 虚拟滚动与分页功能：实现了虚拟滚动机制，处理大数据集，添加了分页控件，支持上一页/下一页切换
- 📂 更新了最新版本的baseGame
...
- 📋 虚拟滚动分页本地化：为虚拟滚动机制添加了分页本地化设置
  - 在主页面设置中添加了分页配置选项，供用户修改
  - 用户可在config.jsonc中设置横列、竖列表格各自的每页数量
  - 该参数能正常被config.js加载和读取
- 🔍 搜索功能增强：修复了搜索栏不能搜索未显示页面内容的问题
  - 现在搜索功能基于原始数据进行搜索，能够搜索到所有项，包括未显示在当前页面的项
  - 搜索结果会显示所有匹配的项
