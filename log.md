...
欢迎使用学生时代模组分析工具，目前尚在逐步开发过程中，问题反馈与建议:
...
- 🛠️ 修复了在某些条件规则下，文本替换不正确的问题
- 🛠️ 修复了文件缓存验证失败，导致加载缓慢的问题
- 🛠️ 优化了进度条显示逻辑，会根据实际加载情况平滑更新
- 🛠️ 修复了mod文件上传后没有被更同步到数据库的问题
- ⭐ 优化了提示框规则显示，现在悬停在效果或者前提规则替换的文本会显示其调用的具体规则。
- ⭐ 新增对成就的解析功能，能够显示成就的名称、描述和触发条件。
